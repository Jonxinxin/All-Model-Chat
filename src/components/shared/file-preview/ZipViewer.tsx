...
export const ZipViewer: React.FC<ZipViewerProps> = ({ file, onConvertToContext }) => {
  ...
  const [previewItem, setPreviewItem] = useState<{ name: string; content: string } | null>(null);
  ...
  const handleViewTextEntry = async (entryInfo: ZipEntryInfo) => {
    ...
    if (entryInfo.size && entryInfo.size > 5 * 1024 * 1024) {
      setPreviewItem({
        name: entryInfo.name,
        content: '文件大小超过 5MB，暂不支持在线文本预览，请点击右侧下载按钮查看。',
      });
      return;
    }
    try {
      const text = await entryInfo.entry.async('text');
      setPreviewItem({ name: entryInfo.name, content: text });
    } catch {
  ...
  return (
    <div className="w-full h-full flex flex-col pt-20 sm:pt-22 pb-4 sm:pb-6 px-3 sm:px-6 select-text">
      <div className="w-full h-full flex flex-col rounded-2xl border border-[var(--theme-border-secondary)] bg-[var(--theme-bg-primary)] shadow-2xl overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3 border-b border-[var(--theme-border-secondary)] bg-[var(--theme-bg-secondary)]/60 flex-shrink-0">
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-[var(--theme-bg-accent)] text-white hover:opacity-90 disabled:opacity-50 transition-all shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--theme-border-focus)] cursor-pointer"
        ...
        {contextError && (
          <div className="flex items-center justify-between px-5 py-2 bg-red-500/10 border-b border-red-500/20 text-xs text-[var(--theme-text-danger)]">
            <div className="flex items-center gap-2">
              <AlertCircle size={14} />
